...
do
   echo "$i"
   query='{"from":0,"query":{"bool":{"filter":[{"term":{"status":"published"}}],"must":{"nested":{"path":"name","query":{"multi_match":{"query":"'$i'","fuzziness":"AUTO","fields":["name.text.edge_ngram_search","name.text.ngram_search"],"type":"most_fields"}}}}}}}'
